...
URL=$SAMPLESURL
test ! -z $ALTURL && URL=$ALTURL #override if needed
echo "Fetching $URL/$SAMPLES..." 
test -e $SAMPLES && echo "Using cached copy" || wget --progress=bar:force $URL/$SAMPLES -O $SAMPLES
echo "Extracting $SAMPLES..." 
unzip -qod $INPUT $SAMPLES
#We only use html4 samples
rm -fr $SAMPLEDIR/../xhtml1/*
rm -fr $SAMPLEDIR/../xhtml1print/*
rm -f $FUZZDIR/support
ln -fs $SAMPLEDIR/support $FUZZDIR/support
